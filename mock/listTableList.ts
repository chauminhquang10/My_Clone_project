...
let listMachine = genListMachine(1, 100);

function getListMachine(req: Request, res: Response, u: string) {
    let realUrl = u;
    if (
        !realUrl ||
        Object.prototype.toString.call(realUrl) !== "[object String]"
    ...
    let dataSource = [...listMachine].slice(
        ((current as number) - 1) * (pageSize as number),
        (current as number) * (pageSize as number)
    );
    if (params.sorter) {
...
let listGroupAuthorize = genGroupAuthorize(1, 100);

function getGroupAuthorize(req: Request, res: Response, u: string) {
    let realUrl = u;
    if (
        !realUrl ||
        Object.prototype.toString.call(realUrl) !== "[object String]"
    ...
    let dataSource = [...listGroupAuthorize].slice(
        ((current as number) - 1) * (pageSize as number),
        (current as number) * (pageSize as number)
    );
    if (params.sorter) {
    ...
    const result = {
        data: dataSource,
        total: listGroupAuthorize.length,
        success: true,
        pageSize,
        current: parseInt(`${params.current}`, 10) || 1,
    };
...
export default {
    ...
    "GET /api/listMachine": getListMachine,
    "GET /api/listGroupAuthorize": getGroupAuthorize,
};
