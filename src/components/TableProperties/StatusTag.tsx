...
const StatusTag: React.FC<StatusTagProps> = ({
    ...
    const [currentStatus, setCurrentStatus] =
        useState<number>(INITIAL_STATUS_INDEX);

    let checkStatusStyle = STATUS_STYLES.DEFAULT;
    if (type && STATUS_STYLES.hasOwnProperty(type)) {
        checkStatusStyle = STATUS_STYLES[type];
    }
    ...
    const SwitchStatusIcon: React.FC<SwitchIconStatusProps> = ({
        ...
        const [tooltipIndexStatus, setTooltipIndexStatus] =
            useState<number>(currentStatusProp);
        ...
        return (
            <>
                {canSwitch && (
                    <Tooltip
                        placement="bottom"
                        title={changableStatus?.statusItems[tooltipIndexStatus]}
                    >
            ...
            {changableStatus ? (
                <div className={styles.statusContent}>
                    <span className={styles.statusTitle}>
                        {changableStatus.statusItems[currentStatus]}
                    </span>
                    <SwitchStatusIcon
